...
export async function commit(folderPath, message) {
  try {
    const repoPath = path.resolve(folderPath, ".bep");
    const workspace = new Workspace(path.resolve(repoPath));
    if (!workspace.exists("config")) {
      printMessage(`*** Please tell me who you are.
      Run
        bep config user.email "you@example.com"
        bep config user.name "Your Name"`);
      return;
    }
    const config = parseConfig(
      fs
        .readFileSync(path.resolve(folderPath, ".bep", "config"))
        .toString("utf8")
    );
    ...
    if (!authorName || !authorEmail) {
      ...
      printMessage(`\tRun`);
      if (!authorName) {
        printMessage(`\t   bep config user.name "Your Name"`);
      } if(!authorEmail) {
        printMessage(`\t   bep config user.email "you@example.com"`);
      }
