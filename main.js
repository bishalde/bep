...
program
  ...
  .description("Initialize empty Bep repository")
  .action(async () => {
    await init(repoFolderPath);
    printMessage("Initialized empty bep repository", MESSAGE_TYPES.NEUTRAL);
  });
program
  ...
  .action(async (paths) => {
    if (isValidRepo(repoFolderPath)) {
      ...
      printMessage("fatal: Not a valid bep repository", MESSAGE_TYPES.NEUTRAL);
    }
  });
